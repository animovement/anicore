...
* A joint frame, `anijoint` (#154): `as_anijoint()` computes one `angle` per joint of a structure from its two segments, signed in 2D, the included angle in 3D, or signed about a joint `axis` (`"x"`, `"y"`, `"z"` or a segment) after projecting onto the plane perpendicular to it. It takes an anisegment or an anipoint, follows `unit_angle`, and works with `convert_unit_angle()` and `convert_unit_time()`. The underlying computation is exported as `angle_between()`.
