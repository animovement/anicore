...
Positions are rebuilt by walking the segments outward from the
structure's `root`, which needs the root point's trajectory:

```{r}
back <- as_anipoint(seg, root = skeleton)
back
```
